...
function getItems($city, $id, $filters){
    ...
    $itemsComments = $xpath->query('//tr[@itemscope]/td[@class="co"]/a');
    ...
    $itemsGrade = $xpath->query('//tr[@itemscope]/td[@class="r"]');
    $nnn=0;
    ...
    foreach($items as $inner) {
        ...
        if($id=="106"){$id="185";}
        $result['Items']["t$i"] = array('Parent'=> $id, 'Code'=>$code[$i], 'Comments'=>$comments[$i], 'Name'=>$name[$i], 'Price'=>$pr, 'Grade'=>$grade[$i], "Preview"=>$inner->getAttribute("preview"));
        $i++;
    }
    curl_close($curl);
...
function getItem($city, $id){
    ...
    $imgs = $xpath->query('//a[@rel="lightbox_catalog_item"]');foreach($imgs as $inner) {$imgsC[]=$inner->getAttribute(href);}
    //Характеристики
    $featp = $xpath->query('//tr[@class="primary"]/td');foreach($featp as $inner) {$feaC[]=$inner->nodeValue;}
    $feats = $xpath->query('//tr[@class="secondary"]/td');foreach($feats as $inner) {$feaC[]=$inner->nodeValue;}
    //Комментарии
    $commm = $xpath->query('//div[@class="message"]/div[@class="box"]');foreach($commm as $inner) {$commmC[]=$inner->nodeValue;}
    ...
    if(count($imgC)>0)
       ...
       $print['Image'] = "";
    for($p=0;$p<count($imgsC);$p++){
        $print['Images']["t".$p] = $imgsC[$p]; 
    }
    $pi=0;
    for($p=0;$p<count($feaC);$p++){
        $print['Fea'][$pi]["key"] = $feaC[$p];
        $print['Fea'][$pi]["value"] = $feaC[$p+1];
        $p++;
        $pi++;
    }
    for($p=0;$p<count($commmC);$p++){
        $print['Comments']["t".$p] = $commmC[$p]; 
    }
...
function getItemsHtml($curl, $city, $id, $filters, $errCount = 1){
    $url = "http://".$city.".dns-shop.ru/catalog/".$id."/?flex=on&fl=on&length_3=0".$filters;
